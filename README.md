Hivenews is a simple News webserver where users can read and explore news posted Admins have access to all the endpoints including news and only users with the admin role can post news.
...
Endpoints:
 ...
 Login / user log in User provide email and password Login compares details given with database records if the the details given is accurate, it will login the user, else login will reject with "invalid credentials" as the returning message 
...
How to run 
this app is live on render url :
